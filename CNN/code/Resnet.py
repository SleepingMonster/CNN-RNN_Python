...
class BasicBlock(nn.Module):
    def __init__(self, in_channels, channels, stride=1, downsample=None):
        ...
        self.conv1 = nn.Conv2d(in_channels=in_channels, out_channels=channels, kernel_size=3, stride=stride, padding=1)
        self.bn1 = nn.BatchNorm2d(num_features=channels)
        self.relu = nn.ReLU(inplace=True)   # inplace=true表示修改回原来的值
    ...
    def forward(self, x):
        ...
        if self.downsample is not None:     # 欠采样
...
class ResNet18(nn.Module):
    ...
    def make_layer(self, block, channels, blocks, stride=1):    # block是表示block类型，是basicblock还是bottleneck（本次不用）
        # blocks是layers数
        downsample = None
